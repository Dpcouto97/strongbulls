                            ...
                            <el-table-column align="center" width="140" class-name="left-gap">
                                <template #header>
                                    <div v-if="can_create" class="flex items-center justify-left">
                                        <button class="icon-button add-button ml-4" @click="addItem" title="Add new user">
                                ...
                                <template #default="{ row }">
                                    <div class="row-actions">
                                        <button v-if="row.can_edit" class="icon-button edit-button" @click="editItem(row)" title="Edit">
...
const tableColumns = [
    ...
    { label: "Email", property: "email", icon: emailIcon },
    {
        label: "Grupo",
        property: "user_group_id",
        icon: groupIcon,
        formatter: (row) => (row.user_group_id && row.user_group_id ? row.user_group.name : ""),
    },
];
...
const getGroupsList = async () => {
    //Busca a lista de Grupos da tabela da BD.
    try {
...
const deleteItem = async (id) => {
    try {
        ...
        await ElMessageBox.confirm("Are you sure you want to delete this user?", {
